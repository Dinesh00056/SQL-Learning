...
select *,(select sum(saleamount) from employeeSales) from employeesSales;
...
max(saleAmount)
over(partition by department) from employeeSales;
...
select *, sum(saleamount) over(order by employeeName desc) from employeeSales;
