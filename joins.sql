...
join enrollment as e on s.studentid=e.studentid; --- inner join
...
select s.studentid,s.studentname, e.enrollmentid from student as s 
join enrollment as e on s.studentid=e.studentid; -- inner join
     -- find the student who didnot enroll in any course
select s.studentid,s.studentname, e.enrollmentid from student as s 
 left join enrollment as e on s.studentid=e.studentid where enrollmentid is null; -- left join                             

select s.studentid,count(e.enrollmentid) from student as s 
left join enrollment as e on s.studentid=e.studentid
group by s.studentid; -- left join

-- students who enroller in course
...
group by e.coursename; -- inner join
